function W = exact_triangulation_2(PPM, imagePoints, error)
...
%         int_point(:,:,i) = midrad(imagePoints(:,:,i),[error error]);
        int_point(:,:,i) = midrad(imagePoints(:,:,i),[error(:,1,i) error(:,2,i)]);
    end
else
    for i=1:numberOfViews
%         int_point(:,:,i) = midrad(imagePoints(:,:,i),[error]);
        int_point(:,:,i) = midrad(imagePoints(:,:,i),[error(:,:,i)]);
    end
end
...
function V = calculate_exact_solution(PPM, correspondences)
    ...
    for i = 1:numberOfViews
        ...
        c = - inv(PPM(:,1:3,i))*PPM(:,4,i);
        
        r1 = [c] +  10*[inv(PPM(:,1:3,i))* [inf(correspondences(1,1,i)), inf(correspondences(1,2,i)) 1]' ]; % basso sn
        r2 = [c] +  10*[inv(PPM(:,1:3,i))* [inf(correspondences(1,1,i)), sup(correspondences(1,2,i)) 1]' ]; % alto sn
        
        r3 = [c] +  10*[inv(PPM(:,1:3,i))* [sup(correspondences(1,1,i)), inf(correspondences(1,2,i)) 1]' ]; % basso dx
        r4 = [c] +  10*[inv(PPM(:,1:3,i))* [sup(correspondences(1,1,i)), sup(correspondences(1,2,i)) 1]' ]; % alto dx
...
fclose(fid);        
% cd(currentdir);
cd('/media/crubino/OS/Users/CRubino/codeEllRec_pami');
end
